...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        total_training_steps = int(len(self.train_dataloader) * self.config.trainer.total_epochs)
    ...
    def _save_checkpoint(self):
        ...
        for name in os.listdir(base_dir):
            ...
            if os.path.isdir(folder_path) and name.startswith("global_step_"):
                try:
                    step_num = int(name.split("_")[-1])
                except ValueError:
                    continue

                ckpt_dir = os.path.join(folder_path, "actor")
                latest_file = os.path.join(base_dir, "latest_checkpointed_iteration.txt")

                # 1. 检查是否需要合并模型分片
                merged_file = os.path.join(ckpt_dir, "huggingface", "model.safetensors.index.json")
                if not os.path.exists(merged_file):
                    if os.path.exists(latest_file):
                        with open(latest_file, "r") as f:
                            latest = f.read().strip()
                        if latest != str(step_num):
                            print(f"{ckpt_dir} 开始合并 DeepSpeed ZeRO 分片...")
                            hf_model_path=self.config.actor_rollout_ref.model.path
                            subprocess.run([
                                "python", "/mnt/shared-storage-user/maqinghe/DeepEyes/scripts/model_merger.py",
                                "--hf_model_path", hf_model_path,
                                "--local_dir", ckpt_dir
                            ], check=True)
                            print("合并完成！")
                        else:
                            print(f"latest={latest} == step={step_num}，跳过合并。")
                    else:
                        print(f"未找到 {latest_file}，跳过合并。")
                else:
                    print(f"检测到已存在完整模型分片 {merged_file}，跳过合并。")
                ...
                if os.path.exists(latest_file):
                    with open(latest_file, "r") as f:
                        latest = f.read().strip()
                    if latest != str(step_num):
                        print(f"latest={latest} 与 step={step_num} 不一致，清理 {ckpt_dir} 下的 .pt 文件...")
                        for file in os.listdir(ckpt_dir):
                            if file.endswith(".pt"):
                                os.remove(os.path.join(ckpt_dir, file))
                    else:
                        print(f"latest={latest} 与 step={step_num} 一致，保留 .pt 文件。")
                else:
                    print(f"未找到 {latest_file}，跳过 STEP 校验。")
            

                if (step_num % self.config.trainer.remain_freq != 0) and (step_num != global_steps):
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                '''
                ['input_ids', 'attention_mask', 'position_ids', 'data_source', 'ability', 'env_name', 'reward_model', 'extra_info', 'rationale', 'origin_multi_modal_data', 'multi_modal_data', 'multi_modal_inputs', 'raw_prompt_ids', 'raw_prompt', 'index']
                '''
                ...
                ['data_source', 'ability', 'reward_model', 'extra_info', 'rationale', 'index']
                '''
                ...
                with _timer("step", timing_raw):
                    ...
                    '''
                    batch.non_tensor_batch.keys
                    ['data_source', 'ability', 'reward_model', 'extra_info', 'rationale', 'index', 'uid', 'multi_modal_inputs', 'tool_cnt']
                    modify /work/basicData/hpfs1/zhaozhen/MLLM/DeepEyes/verl/trainer/ppo/metric_utils.py line 177  reserve 'tool_cnt'
                    '''
                    ...
                    print('Finish computing old log_probs ...')
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        try:
                            reward_result = self.reward_fn(batch, return_dict=True, epoch=self.global_steps)
                ...
                '''
                batch.batch
                [\'position_ids\', \'prompts\', \'attention_mask\', \'env_reward\', \'responses\', \'input_ids\', \'action_mask\', \'response_mask\', \'old_log_probs\', \'token_level_scores\', \'token_level_rewards\', \'advantages\', \'returns\']
                non_tensor_batch
                ['data_source', 'ability', 'reward_model', 'extra_info', 'rationale', 'index', 'uid', 'multi_modal_inputs', 'score', 'detail']
                '''
