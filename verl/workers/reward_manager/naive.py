...
class NaiveRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict=False, epoch=None):
        ...
        for i in range(len(data)):
            ...
            if response_str.strip().startswith("<tool_on>"):
                tool_on_count += 1
            elif response_str.strip().startswith("<tool_off>"):
                tool_off_count += 1
            else:
                invalid_count += 1
        total = tool_on_count + tool_off_count + invalid_count
        tool_on_ratio = tool_on_count / (tool_on_count + tool_off_count) if tool_on_count + tool_off_count > 0 else 0
        print(f'[REWARD DEBUG] {tool_on_count=} {tool_off_count=} {invalid_count=} {tool_on_ratio=}')

        for i in range(len(data)):
            if (i+1) % (len(data)//10) == 0:
                print(f'[Debug] {len(data)=} {i=}')
